...
| YouTube | 7.1 | First pass | [youtube](blueprints/youtube.md) |
...
An explicitly opted-in `level_contract: stage-plan` is a breaking alternative for staged experiments. It publishes one contiguous `## Stage Roadmap`, where every stage combines bounded work with its own expected result and trigger. A stage closes only when required work and its own trigger pass; optional work never blocks completion. Capability references are integrated into Required/Optional task bullets as known catalog slugs in backticks, and any conditional condition stays in the same task sentence; the roadmap has no duplicate capability cards. The review clock starts at stage entry or the last review and is separate from the evidence window. Exceptions require owner approval and a private record; falling evidence does not automatically regress or pause a stage, and built work is retained after graduation. The reusable block shape is documented in [`docs/stage-template.md`](docs/stage-template.md). YouTube 7.1 uses E0–E5 and leaves later stages undefined until a reviewed blueprint version publishes them.
